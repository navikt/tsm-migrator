...
const val uke17Prefix = "6.4"
const val uke39Prefix = "6.5"


fun spmMapping(prefix: String): Map<Sporsmalstype, Pair<String, String>> = mapOf<Sporsmalstype, Pair<String, String>>(
    ...
    val receivedSykmelding = ReceivedSykmelding(
        sykmelding = SykmeldingLegacy(
            ...
            medisinskVurdering = MedisinskVurdering(
                ...
                biDiagnoser = sykmelding.medisinskVurdering.biDiagnoser?.let { bidagnoser -> bidagnoser.map { it.toDiagnose() } }
                    ?: emptyList(),
            ...
            avsenderSystem = AvsenderSystem(
                sykmelding.metadata.avsenderSystem.navn, versjon = sykmelding.metadata.avsenderSystem.versjon
            ),
...
private fun AnnenFravarsgrunn?.toAnnenFraversArsak(): AnnenFraversArsak? {
    ...
    return AnnenFraversArsak(
        null, grunn = listOf(annenFraversArsakType)
    )
}
...
private fun PersonId.toXmlIdent(): XMLIdent? {
    val type = XMLCV()
    val (verdi, navn) = getIdTypeAndName()
    if (verdi == null) return null

    type.dn = navn
    type.v = verdi
    type.s = "2.16.578.1.12.4.1.1.8116"
...
private fun PersonId.toIdent(): Ident? {
    val type = CV()
    val (verdi, navn) = getIdTypeAndName()
    if (verdi == null) return null
...
private fun PersonId.getIdTypeAndName(): Pair<String?, String?> = when (this.type) {
    PersonIdType.FNR -> "FNR" to "Fødselsnummer"
    PersonIdType.DNR -> "DNR" to "D-nummer"
    PersonIdType.HNR -> "HNR" to "H-nummer"
    PersonIdType.HPR -> "HPR" to "HPR-nummer"
    PersonIdType.HER -> "HER" to "HER-id"
    PersonIdType.PNR -> "PNR" to "Passnummer"
    PersonIdType.SEF -> "SEF" to "Svensk 'personnummer'"
    PersonIdType.DKF -> "DKF" to "Dansk 'personnummer'"
    PersonIdType.SSN -> "SSN" to "Sosial security number"
    PersonIdType.FPN -> "FPN" to "Forsikringspolise nummer"
    PersonIdType.DUF -> "DUF" to "DUF-nummer"
    PersonIdType.XXX -> "XXX" to "Annet"
    PersonIdType.UGYLDIG -> "" to "Ugyldig"
                    ...
                    sender = XMLSender().apply {
                        ...
                        organisation = XMLOrganisation().apply {
                            healthcareProfessional = XMLHealthcareProfessional().apply {
                                ...
                                ident.addAll(sykmelding.behandler.ids.mapNotNull {
                                    it.toXmlIdent()
                                })
                            }
                        }
                                    ...
                                    HelseOpplysningerArbeidsuforhet().apply {
                                        ...
                                        pasient = HelseOpplysningerArbeidsuforhet.Pasient().apply {
                                                navn = NavnType().apply {
                                                    ...
                                                    mellomnavn = sykmelding.pasient.navn?.mellomnavn
                                                    etternavn = sykmelding.pasient.navn?.etternavn
                                                }
                                        ...
                                        meldingTilNav = sykmelding.bistandNav?.let {
                                                HelseOpplysningerArbeidsuforhet.MeldingTilNav().apply {
                                                        beskrivBistandNAV = it.beskrivBistand
                                                        isBistandNAVUmiddelbart = it.bistandUmiddelbart
                                                    }
                                            }
                                        meldingTilArbeidsgiver = when (val arbeidsgiver = sykmelding.arbeidsgiver) {
                                            is IngenArbeidsgiver -> null
                                        ...
                                        kontaktMedPasient = HelseOpplysningerArbeidsuforhet.KontaktMedPasient().apply {
                                                ...
                                                begrunnIkkeKontakt = sykmelding.tilbakedatering?.begrunnelse
...
fun tilBehandler(sykmelder: no.nav.tsm.sykmelding.input.core.model.Behandler): HelseOpplysningerArbeidsuforhet.Behandler =
    HelseOpplysningerArbeidsuforhet.Behandler().apply {
        ...
        id.addAll(
            sykmelder.ids.mapNotNull { it.toIdent() }
        )
        adresse = Address()
        kontaktInfo.add(
            TeleCom().apply {
...
fun tilHelseOpplysningerArbeidsuforhetPeriode(
    periode: Periode
): HelseOpplysningerArbeidsuforhet.Aktivitet.Periode = HelseOpplysningerArbeidsuforhet.Aktivitet.Periode().apply {
...
fun tilMedisinskVurdering(
    ...
    return HelseOpplysningerArbeidsuforhet.MedisinskVurdering().apply {
        if (hovedDiagnose != null) {
            this.hovedDiagnose = HelseOpplysningerArbeidsuforhet.MedisinskVurdering.HovedDiagnose().apply {
...
): HelseOpplysningerArbeidsuforhet.Arbeidsgiver = HelseOpplysningerArbeidsuforhet.Arbeidsgiver().apply {
    harArbeidsgiver = when (arbeidsgiver.type) {
        ARBEIDSGIVER_TYPE.INGEN_ARBEIDSGIVER -> CS().apply {
            dn = "Ingen arbeidsgiver"
            v = "3"
        }

        ARBEIDSGIVER_TYPE.FLERE_ARBEIDSGIVERE -> CS().apply {
            dn = "Flere arbeidsgivere"
            v = "2"
        }

        ARBEIDSGIVER_TYPE.EN_ARBEIDSGIVER -> CS().apply {
            dn = "Én arbeidsgiver"
            v = "1"
        }
    }
...
fun mapToMerknader(validation: ValidationResult): List<Merknad>? {
    ...
    val tilbakedatertMerknad =
        TilbakedatertMerknad.entries.firstOrNull { it.name == lastRule.name }?.toOldTilbakedateringMerknad()
            ?: return null
